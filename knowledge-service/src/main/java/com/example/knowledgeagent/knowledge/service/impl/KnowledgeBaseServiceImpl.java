...
public class KnowledgeBaseServiceImpl implements KnowledgeBaseService {
    ...
    public void deleteKnowledgeBase(Long id) {
        mustGet(id);
        // 知识库服务不再跨 schema 清理文档和向量数据；关联资源由 document-service 通过内部接口或补偿任务处理。
    ...
    public void updateStatistics(Long knowledgeBaseId) {
        ...
        // 全面微服务后，统计数据由 document-service 通过显式 DTO 推送；本方法仅保留兼容入口。
        entity.setUpdatedAt(LocalDateTime.now());
        knowledgeBaseMapper.updateById(entity);
    }

    /**
     * 使用文档服务推送的统计值更新知识库冗余字段，避免 knowledge-service 直接查询 document schema。
     */
    @Transactional
    public void updateStatistics(Long knowledgeBaseId, Integer documentCount, Integer chunkCount) {
        KnowledgeBase entity = mustGet(knowledgeBaseId);
