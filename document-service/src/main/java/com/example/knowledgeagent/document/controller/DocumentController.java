...
public class DocumentController {
    ...
    @SentinelResource(value = "document.upload")
    ...
    /**
     * 为前端直传 MinIO 生成短期 PUT 地址，文件内容不会经过 Gateway 或 document-service。
     */
    @PostMapping("/knowledge-bases/{knowledgeBaseId}/documents/upload-url")
    public ApiResult<PresignedUploadUrlResponse> createUploadUrl(@PathVariable Long knowledgeBaseId,
                                                                 @Validated @RequestBody CreatePresignedUploadUrlRequest request) {
        return ApiResult.ok(documentService.createPresignedUploadUrl(knowledgeBaseId, request));
    }

    /**
     * 前端直传完成后的确认接口，负责校验 MinIO 对象并创建文档和索引任务。
     */
    @PostMapping("/knowledge-bases/{knowledgeBaseId}/documents/complete-upload")
    @SentinelResource(value = "document.complete-upload")
    public ApiResult<DocumentUploadResponse> completeUpload(@PathVariable Long knowledgeBaseId,
                                                            @Validated @RequestBody CompletePresignedUploadRequest request) {
        return ApiResult.ok(documentService.completePresignedUpload(knowledgeBaseId, request));
    }
    ...
    @SentinelResource(value = "document.reindex")
